...
contract digiLocker {
    ...
    event uploadDocumentEvent(
        bytes32 indexed docid,
        bytes32 docHash,
        address indexed user_addr
    );
    event sharedDocumentEvent(
        bytes32 indexed docid,
        address indexed docOwner,
        address indexed sharedWith,
        uint8 permission
    );
    event verifyDocumentEvent(
        bytes32 indexed docid,
        address indexed docOwner,
        address indexed sharedWith
    );
    ...
    function getRegisteredUser()
        public
        view
        returns (bytes32, address, string memory)
    {
        return (
    ...
    function registerUser(
        ...
        UserDetails memory d = UserDetails(
            _firstName,
            _lastName,
            _email,
            _contact
    ...
    function checkAlreadyUpload(bytes32 docId) public view returns (bool) {
        for (uint i = 0; i < ownerDocuments[msg.sender].length; i++)
            if (ownerDocuments[msg.sender][i].docid == docId) return true;
        return false;
    }
    ...
    function shareDocumentwithUser(
        bytes32 docid,
        uint8 permission,
        address _requester
    ) public {
    ...
    function isValidSharableUser(
        string memory email_
    ) public view returns (bool) {
        if (
            emailAddressMapping[email_] !=
            0x0000000000000000000000000000000000000000 &&
    ...
    function getOwnerDocInfoByDocId(
        bytes32 docId
    ) public view returns (string memory, string memory) {
        for (uint i = 0; i < ownerDocuments[msg.sender].length; i++) {
            if (ownerDocuments[msg.sender][i].docid == docId)
                return (
                    ownerDocuments[msg.sender][i].docName,
                    ownerDocuments[msg.sender][i].timestamp
                );
        }
    }

    function getOwnerDocumetList()
        public
        view
    ...
    function getDocumetList(
        address _useradd
    ) public view returns (string[] memory, string[] memory, bytes32[] memory) {
        string[] memory _docName = new string[](
            ownerDocuments[_useradd].length
        );
        string[] memory _timestamp = new string[](
            ownerDocuments[_useradd].length
        );
        bytes32[] memory _docid = new bytes32[](
            ownerDocuments[_useradd].length
        );
    ...
    function getDocumentListbyDocId(
        bytes32 _docId
    )
        public
        view
        ...
        )
    {
        for (uint i = 0; i < usercount; i++) {
            for (
                uint j = 0;
                j < ownerDocuments[_glbluseraddress[i]].length;
                j++
            ) {
    ...
    function getEmailIdByAddrss()
        public
        view
        returns (string memory, string memory, string memory)
    {
        return (
            registerUsers[msg.sender].details.email,
            registerUsers[msg.sender].details.firstName,
    ...
    function getEmailIdByUsrAddr(
        address _usraddrs
    ) public view returns (string memory, string memory, string memory) {
        return (
            registerUsers[_usraddrs].details.email,
